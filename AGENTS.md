...
- The spec pins cross-cutting invariants, decisions with their WHY, and compatibility classes (e.g. "JSON summary documents evolve additively"). It never inventories output field names, exact wording, or rendering details - those are pinned where they are enforced: the serde types that emit them and the integration tests. A spec touch for a behavior change is one to three sentences on the relevant bullet or invariant, stated as a class, not a field list.
...
- `pnpm dev` (vocs dev, port 5173) is the working loop - MDX edits hot-update, and a broken MDX file logs a compilation error and keeps serving instead of taking the server down. Verify a final state against `pnpm build` + a static server over `dist/public`, since that is what ships. Clipboard APIs need a secure context: test copy flows over HTTPS (`tailscale serve`) or localhost, never over LAN-IP HTTP.
...
- **Body** = the full PR description, landed automatically at merge by the repo's squash defaults (`squash_merge_commit_title: PR_TITLE`, `squash_merge_commit_message: PR_BODY` - repo settings, LOAD-BEARING: verify with `gh api repos/tenequm/pond --jq '{title: .squash_merge_commit_title, message: .squash_merge_commit_message}'`, restore with `gh api -X PATCH repos/tenequm/pond -f squash_merge_commit_title=PR_TITLE -f squash_merge_commit_message=PR_BODY`). The changelog template extracts the `## Release note` section itself; the rest of the description rides along in git history but never reaches the changelog. Write the section for pond users: ONE paragraph, at most 300 characters, no bullets and no blank lines (CI enforces all three), saying what changed for them and what they must do - not what you did or how you validated it; ASCII only, no leading indentation (the template indents it), and no HTML comments inside the section (the template cannot strip them; CI rejects them). The cap is calibrated against Kubernetes' `release-note` block (median 132 chars over 24 sampled merged PRs); anything longer belongs above the section or in the `[release-note]` lead, which is exempt. A body with `## ` sections but no `## Release note` contributes no prose; a body with no sections at all (a plain direct commit) renders whole, as before.
...
- **Merge** with a plain `gh pr merge <N> --squash` - the repo squash defaults produce the right subject and body from the PR title and description. Never pass `-b`/`-F` to override the body, and never let a client substitute the intermediate-commit list. A body that goes missing anyway is recovered at generation time (see the self-repair note below), so this is hygiene, not a load-bearing step.
...
- **A squash that loses the body repairs itself.** The prose lives in the commit body, and a merge that submits an empty description field overrides the repo default and drops it - immutably, since git-cliff exposes `pr_title`/`pr_number`/`pr_labels` but no `pr_body` to fall back on (happened on #191). So the first `commit_preprocessor` runs `ops/scripts/release-note-from-pr.sh`: for a `(#N)` subject carrying no `## Release note`, it fetches the section from that PR and appends it before parsing. Changelog and GitHub release body both render from this, so both stay correct however the merge was done - and a note can be corrected after merge by editing the PR description. Every failure path (no `gh`, no auth, no network) passes the message through untouched, so local `release-plz update` still works.
